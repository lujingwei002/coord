...
public:
    ...
    ~object_pool(){
        //CoreDebugln("ObjectPool<%s>::~ObjectPool", this->name.c_str());
    ...
    pointer alloc() {
        ...
        //CoreDebugln("[ObjectPool<%s>] alloc, address=%p", this->name.c_str(), object);
        this->freeList.pop_back();
    ...
    pointer construct() {
        ...
        //CoreDebugln("[ObjectPool<%s>] construct", this->name.c_str());
    ...
    bool free(pointer object) {
        ...
        if (it == this->allocateList.end()){
            CoreErrorln("ObjectPool<%s> free failed, address=%p", this->name.c_str(), object);
            return false;
        }
        //CoreDebugln("ObjectPool<%s> free, address=%p, freeList:%d", this->name.c_str(), object, this->freeList.size());
    ...
    virtual void trace() {
        if (this->allocateList.size() != 0) {
            CoreDebugln("freeList:%02d allocateList:%02d, ObjectPool<%s> ", this->freeList.size(), this->allocateList.size(), this->name.c_str());
        } else {
            CoreDebugln("freeList:%02d allocateList:%02d, ObjectPool<%s>", this->freeList.size(), this->allocateList.size(), this->name.c_str());
        }
    }
