...
int cluster_redis_config::checkNodeStatusAsync(std::string nodeName, uint64_t version) {
    ...
    promise->Then([this, nodeName, version](redis::AsyncClient* client, const redis::RedisResultRef& reply) {
        if (reply->Empty()) {
            ...
            promise->Else([this, nodeName, version](redis::AsyncClient* client, const redis::RedisResultRef& reply) {
                this->coord->CoreLogError("[cluster_redis_config] checkNodeStatusAsync failed, function='this->asyncClient->HDEL', group=%s, node=%s, version=%ld", 
                this->group.c_str(), nodeName.c_str(), version);
            });       
        } else {
...
int cluster_redis_config::checkNodeStatusAsync() {
    ...
    promise->Then([this](redis::AsyncClient* client, const redis::RedisResultRef& reply){
    ...
    promise->Else([this](redis::AsyncClient* client, const redis::RedisResultRef& reply){
