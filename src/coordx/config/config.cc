#include "coordx/config/config.h"
...
Config::Config(Coordx* coordx) {
    this->coordx = coordx;

    this->Basic.GC = 0;
    this->Basic.Name = "coordx";
...
int Config::gotConfigLineError(const std::string& configPath, int lineNum, char* data, size_t size) {
    ...
    this->coordx->CoreLogError("parse error %s(%d): %s", configPath.c_str(), lineNum, line.c_str());
    return 0;
}
    ...
    for (; i < size; i += codePointLength) {
        ...
        if (phase == 0) {
            ...
            if (IS_ASCII('}', c, codePointLength)) {
                ...
                if (this->coordx->Environment->Get(name, realValue)) {
...
int Config::scanConfigFile(const std::string& configPath) {
    ...
    if (nullptr == f) {
        this->coordx->CoreLogError("no such file or directory: %s", configPath.c_str());
...
void Config::DebugString(){
    this->coordx->LogInfo("========================= Config =========================");
    for (auto const & it : this->Sections) {
        this->coordx->LogInfo("[%s] [%s]", TAG, it.first.c_str());
        for (auto const & it1 : it.second) {
            this->coordx->LogInfo("[%s] %-18s = %s", TAG, it1.first.c_str(), it1.second.c_str());
        }
    }
    this->coordx->LogInfo("========================= [%s] =========================", DEFAULT_SECTION_NAME.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "env", this->Basic.Env.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "name", this->Basic.Name.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "main", this->Basic.Main.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "scene", this->Basic.Scene.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "registery", this->Basic.Registery.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "package", this->Basic.Package.c_str());
    this->coordx->LogInfo("[%s] %-15s: %ld", TAG, "gc", this->Basic.GC);
    this->coordx->LogInfo("[%s] %-15s: %ld", TAG, "update", this->Basic.Update);
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "worker", this->Basic.Worker.c_str());
    this->coordx->LogInfo("[%s] %-15s: %ld", TAG, "worker-num", this->Basic.WorkerNum);
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "proto", this->Basic.Proto.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "version", this->Basic.Version.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "short-version", this->Basic.ShortVersion.c_str());

    this->coordx->LogInfo("========================= [%s] =========================", WEB_SECTION_NAME.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "host", this->Web.Host.c_str());
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "port", this->Web.Port);
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "backlog", this->Web.Backlog);
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "recv-buffer", this->Web.RecvBuffer);
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "ssl-encrypt", this->Web.SSLEncrypt ? "true" : "false");
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "ssl-pem", this->Web.SSLPemFile.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "ssl-key", this->Web.SSLKeyFile.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "asset-dir", this->Web.AssetDir.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "use-etag", this->Web.UseEtag? "true" : "false");

    this->coordx->LogInfo("========================= [%s] =========================", GATE_SECTION_NAME.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "sever-name", this->Gate.ServerName.c_str());
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "port", this->Gate.Port);
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "host", this->Gate.Host.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "network", this->Gate.Network.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "rsa-encrypt", this->Gate.RsaEncrypt? "true" : "false");
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "rsa-keyfile", this->Gate.RsaKeyFile.c_str());
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "heartbeat", this->Gate.Heartbeat);
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "backlog", this->Gate.Backlog);
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "recv-buffer", this->Gate.RecvBuffer);
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "ssl-encrypt", this->Gate.SSLEncrypt ? "true" : "false");
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "ssl-pem", this->Gate.SSLPemFile.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "ssl-key", this->Gate.SSLKeyFile.c_str());
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "max-user", this->Gate.MaxUser);
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "max-connection", this->Gate.MaxConnection);
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "cluster", this->Gate.Cluster.c_str());
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "register-expire", this->Gate.RegisterExpire);
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "register-interval", this->Gate.RegisterInterval);

    this->coordx->LogInfo("========================= [%s] =========================", CACHE_SECTION_NAME.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "password", this->Cache.Password.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "db", this->Cache.DB.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "host", this->Cache.Host.c_str());
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "port", this->Cache.Port);

    this->coordx->LogInfo("========================= [%s] =========================", CLUSTER_SECTION_NAME.c_str());
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "port", this->Cluster.Port);
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "host", this->Cluster.Host.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "driver", this->Cluster.Driver.c_str());
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "heartbeat", this->Cluster.Heartbeat);
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "expire", this->Cluster.Expire);
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "reload-interval", this->Cluster.ReloadInterval);
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "reconnect-interval", this->Cluster.ReconnectInterval);
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "register-interval", this->Cluster.RegisterInterval);
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "name", this->Cluster.Name.c_str());
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "recv-buffer", this->Cluster.RecvBuffer);

    this->coordx->LogInfo("========================= [%s] =========================", MANAGED_SECTION_NAME.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "host", this->Managed.Host.c_str());
    this->coordx->LogInfo("[%s] %-15s: %d", TAG, "port", this->Managed.Port);
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "user", this->Managed.User.c_str());
    this->coordx->LogInfo("[%s] %-15s: %s", TAG, "password", this->Managed.Password.c_str());
...
int Config::parse(const std::string&  configPath) {
    ...
    if (err) {
        this->coordx->CoreLogError("%s: %s", uv_strerror(err), configPath.c_str());
        return err;
    }
        ...
        while (this->Basic.Package.length() > 0) {
            ...
            if (pos == std::string::npos) {
                ...
                if (!coord::path::IsAbsolutePath(path.c_str())) {
                    path = coord::path::PathJoin(this->coordx->Environment->ConfigDir, path);
                }
                if (package.length() == 0) {
                    package = path;
                } else {
