    ...
    -- return values
        if self.type ~= '' and self.type ~= 'void' then
            ...
            if t then
				...
				--print(t)
                if self.ptr == '' then
					if string.find(t, '^coordx::RcRef') then
						-- 对RcRef类型特殊处理
						local rc = string.gsub(t, '^coordx::RcRef<', '')
						rc = string.gsub(rc, '>$', '')
						output('   {')
						output('#ifdef __cplusplus\n')
						output('    void* tolua_obj = tolua_ret.TakeOwnerShip();')
						output('    tolua_pushusertype_and_takeownership(tolua_S,tolua_obj,"',rc,'");')
						output('#else\n')
						output('    not support\n')
						output('#endif\n')
						output('   }')
...
function classFunction:requirecollection (t)
	...
	if self.type ~= '' and not isbasic(self.type) and self.ptr=='' then
		...
		if string.find(type, '^coordx::RcRef') then
		else
